...
class List{
    updateNoOftasks(){
        ...
        // let tasksLeft=document.querySelector(".no-of-items");
         allTasksLeft.innerText=`${document.querySelector(".all-tasks").children.length}`;   
         activeTasksLeft.innerText=`${document.querySelector(".active-tasks").children.length}`;
compltedTasksLeft.innerText=`${document.querySelector(".completed-tasks").children.length}`;        
...
btn.parentElement.onclick=(e)=>{
    if(e.target.classList.contains("check-btn")&&e.target.parentElement.parentElement.classList.contains("active-tasks")){
...
const ui=new Ui()
        ...
        updateLocalStorage(){
            ...
            const activeTask=[]
            const completedTask=[]
            document.querySelector(".all-tasks").querySelectorAll("li p").forEach((statement)=>{
                generalTask.push(statement.innerText)
            })
            document.querySelector(".active-tasks").querySelectorAll("li p").forEach((statement)=>{
                activeTask.push(statement.innerText)
            })
            document.querySelector(".completed-tasks").querySelectorAll("li p").forEach((statement)=>{
        ...
        retrieveFromLocalStorage(){
            ...
            completedTasks.forEach((task)=>{
               document.querySelector(".all-tasks").querySelectorAll("li").forEach((genTask)=>{
                    if(task==genTask.children[1].innerText){
                        genTask.classList.add("task","li-crossed");
                        genTask.querySelector(".check-btn").classList.add("check-btn-checked");
                        genTask.classList.add("li-crossed");
                        // compare with active tasks to remove
                        document.querySelector(".active-tasks").querySelectorAll("li").forEach((li)=>{
